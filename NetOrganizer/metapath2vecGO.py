...
def genwalk(walks, genesets, walknum, walklen, geneORGANizer, logo):
    ...
    for gene in genesets:  # 对于每个基因，走了20条拥有50个结点的GBGB...
        for i in range(walknum):  # 每个gene节点走20条序列
            # walk = []                                  # GTTTTTTT...(无法迁移)
            # #  将当前gene纳入walk
            # walk.append(gene)
...
def grnwalk(walks, genesets, walknum, walklen, grn, logo):
    ...
    for gene in genesets:
        for i in range(walknum * 5):  # 每个gene拥有20*5条 G-RE-TF-RE-TF...的序列  这个20*5是调参出来的吗？
            walk = []
            walk.append(gene)
...
def main():
    ...
    # global model
    #
    # f = open('./lzm_mp1_result/' + '5_25_1500_full6path_geneORGANizer_metapath' + '_walks' + notename + '.pkl', 'wb')
    # pickle.dump(walks, f)
    # f.close()


    g = open('./lzm_mp1_result/' + '5_25_1500_full7path_geneORGANizer_metapath' + '_walks' + notename + '.pkl', 'rb+')
    walks = pickle.load(g)
    ...
    # lzm_mp1
    # model = Word2Vec(walks, vector_size=200, window=5, min_count=0, sg=1, workers=16, epochs=10)  # 0.53
    # model = Word2Vec(walks, vector_size=200, window=2, min_count=0, sg=1, workers=16, epochs=10)  # 0.52
    model = Word2Vec(walks, vector_size=200, window=10, min_count=0, sg=1, workers=16, epochs=10)
    print('train done')
    model.save('./lzm_mp1_result/' + '5_26_2300_sz10_vec')
    model.wv.save_word2vec_format('./lzm_mp1_result/' + '5_26_2300_sz10_vec.txt')
...
def getauroc(y, prob):
    ...
    plt.savefig("./lzm_mp1_result/AUCs/GO&GRN_5_26_2300_sz10_vec" + ".jpg")
    return roc_auc
...
def test(vecs):
    ...
    f = open('./lzm_mp1_result/' + '5_26_2300_sz10' + '_standard_score'+notename+'.pkl', 'wb')
    pickle.dump((standard, score), f)
    f.close()

    f = open('./lzm_mp1_result/' + '5_26_2300_sz10' + '_standard'+notename+'.pkl', 'wb')
    pickle.dump(standard, f)
    f.close()
    f = open('./lzm_mp1_result/' + '5_26_2300_sz10' + '_score'+notename+'.pkl', 'wb')
    pickle.dump(score, f)
    f.close()
    ...
    with open('./process_result/5_26_2300_sz10_auc.txt', 'w') as f:
...
if __name__=='__main__':
    preprocess()
    main()
    vecs=importvec('./lzm_mp1_result/' + '5_26_2300_sz10_vec.txt')
    test(vecs)
    # use_model()
    # lzm_roc_curve_test()
